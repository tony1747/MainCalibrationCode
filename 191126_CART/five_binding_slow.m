...
g=param(2);
k_n(1:5)=param(6);
k(1:5,1)=param(3); %change 1:4 to 1:5 for 5-binding
k(1,2)=param(4);
k(1,3)=param(5); 

%Try 3:
k(2,1)=param(3)/1;   %this will delete the previous value of k(2,1)
k(2,2)=param(4)*2/3;
k(2,3)=param(4)/3;
k(2,4)=param(5)*2;   %exact values of constants m and n remain to be found
k(3,1)=param(3)/1^2; %here we take m and n to be 2 firstly
k(3,2)=param(4)*3/7;
k(3,3)=param(4)*3/7;
k(3,4)=param(4)/7;
k(3,5)=param(5)*2^2;
...
%k(5,2:5)=1/4*(param(4)*param(5))/(param(4)+param(5));
  %k(5,3)=1/4*(param(4)*param(5))/(param(4)+param(5));
  %k(5,4)=1/4*(param(4)*param(5))/(param(4)+param(5));
  %k(5,5)=1/4*(param(4)*param(5))/(param(4)+param(5));
%k(5,6)=param(4);
%k(5,7)=param(4)*param(5)^2/(param(4)^2+param(5)*param(4));
%Try 1:
%k(2,2)=param(4)*2/3;
%k(2,3)=param(4)/3;
%k(2,4)=param(5);
